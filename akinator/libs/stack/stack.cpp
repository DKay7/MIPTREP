...
int StackPop (Stack* stack, stack_type* out_element)
{
    ...
    {   
        stack->info.error_code |= (unsigned int) err_code;
...
int StackDecrease (Stack* stack)
{
    ...
    {
        stack_type* tmp = (stack_type*) realloc (stack->data, (size_t) (((STACK_DECREASE_COEFFICIENT) * ((double) stack->capacity) + 1)) * sizeof (stack_type));
        ...
        stack->data = tmp;
        stack->capacity = (size_t) ((STACK_DECREASE_COEFFICIENT) * ((double) stack->capacity) + 1);
...
int StackValidate (Stack* stack, unsigned int func_code)
{
    ...
    return (int) stack->info.error_code;
}
...
{   
    ...
    unsigned int err_code = (unsigned) StackValidate (stack, STACK_DUMP_CODE);
